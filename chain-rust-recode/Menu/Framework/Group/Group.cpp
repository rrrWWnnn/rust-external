...
Vector2 override_size = { -1, -1 };

void GUI::Group::BeginGroup( const std::string& name, const Vector2& size ) {
	...
	Vector2 cursor_pos = PopCursorPos( );
	Vector2 group_size;
	...
	if( ( cursor_pos.y - 20 + group_size.y ) > ctx->size.y - 37 ) {
		...
		Vector2 draw_pos = ctx->pos + cursor_pos;
		...
		Render::DirectX::rect_fill( draw_pos + Vector2( 11, 0 ), Vector2( 6 + text_size.x, 2 ), { 17, 17, 17 } );
		...
		PushCursorPos( cursor_pos + Vector2( 22, 23 + scrolling[ id ] ) );

		ctx->parent = "root." + ctx->Tabs[ ctx->ActiveTab ] + "." + name;
		ctx->NextGroupPos = cursor_pos + Vector2( 0, group_size.y + 20 );
...
void GUI::Group::EndGroup( ) {
	...
	if( max_height > VisibleHeight ) {
		...
		bool ScrollBarHovered = InputHelper::IsHoveringOverPoint( gp + Vector2( gs.x - 6, scrollbar_pos ), Vector2( 4, scrollbar_height ) );
		...
		if( !ctx->hovered_listbox && !ctx->MultiDropdownInfo.DraggingScroll && !ctx->DropdownInfo.DraggingScroll ) {
			if( ctx->FocusedID == 0 || ctx->FocusedID == id ) {
				bool bHoveredMain = InputHelper::IsHoveringOverPoint( gp, gs - Vector2( 6, 0 ) );
				bool bHoveredScroll = InputHelper::IsHoveringOverPoint( gp + Vector2( gs.x - 6, scrollbar_pos ), Vector2( 4, scrollbar_height ) );
				...
				if( bHoveredMain || bHoveredScroll || ctx->FocusedID == id ) {
					...
					if( ctx->FocusedID != id ) {
						if( g_Vars.global.scrollbar == 1 ) {
							scrolling[ id ] = scrolling [id] + 2;
						}
						else if( g_Vars.global.scrollbar == -1 ) {
							scrolling[ id ] = scrolling [id] - 2;
						}
					}
				}
	...
	ctx->NextGroupPos = Vector2( 0, 0 );
	Render::DirectX::set_viewport( o_vp );
	Render::DirectX::Fonts::menu_bold.draw_text( name, gp + Vector2( 14, -5 ), { 203, 203, 203 }, 4 );

	if( max_height > VisibleHeight ) {
		Render::DirectX::rect_fill( gp + Vector2( gs.x - 7, 2 ), Vector2( 6, gs.y - 3 ), Color( 45, 45, 45 ) );

		Render::DirectX::rect_fill( gp + Vector2( gs.x - 6, scrollbar_pos ), Vector2( 4, scrollbar_height + 1 ),
			( InputHelper::IsHoveringOverPoint( gp + Vector2( gs.x - 6, scrollbar_pos ), Vector2( 4, scrollbar_height + 1 ) ) || ctx->FocusedID == id ) ? Color( 80, 80, 80 ) : Color( 65, 65, 65 ) );

		// bottom
		Vector2 GradientSize = Vector2( gs.x - 9, 20 );
		Vector2 GradientPos = gp + Vector2( 2, gs.y - GradientSize.y - 1 );
		...
		if( scrolling[ id ] >= ( -max_height + ( int )gs.y ) + 10 ) {
			...
			Render::DirectX::rect_fill( gp + gs - Vector2( 17, 10 ), Vector2( 5, 1 ), Color( 0, 0, 0 ) );
			Render::DirectX::rect_fill( gp + gs - Vector2( 17, 9 ), Vector2( 5, 1 ), Color( 204, 204, 204 ) );
			Render::DirectX::rect_fill( gp + gs - Vector2( 16, 8 ), Vector2( 3, 1 ), Color( 204, 204, 204 ) );
			Render::DirectX::rect_fill( gp + gs - Vector2( 15, 7 ), Vector2( 1, 1 ), Color( 204, 204, 204 ) );
		}

		// top arrow facing up
		if( scrolling[ id ] <= -10 ) {
			// top
			GradientPos = gp + Vector2( 2, 5 );
			Render::DirectX::gradient_v( GradientPos, GradientSize, { 17, 17, 17 }, Color( 17, 17, 17, 0 ) );

			Render::DirectX::rect_fill( gp + Vector2( gs.x - 20, 7 ) + Vector2( 5, 0 ), Vector2( 1, 1 ), Color( 204, 204, 204 ) );
			Render::DirectX::rect_fill( gp + Vector2( gs.x - 20, 7 ) + Vector2( 4, 1 ), Vector2( 3, 1 ), Color( 204, 204, 204 ) );
			Render::DirectX::rect_fill( gp + Vector2( gs.x - 20, 7 ) + Vector2( 3, 2 ), Vector2( 5, 1 ), Color( 204, 204, 204 ) );
			Render::DirectX::rect_fill( gp + Vector2( gs.x - 20, 7 ) + Vector2( 3, 3 ), Vector2( 5, 1 ), Color( 0, 0, 0 ) );
		}
	}
}

void GUI::Group::SetNextSize( const Vector2& size ) {
	override_size = size;
}
