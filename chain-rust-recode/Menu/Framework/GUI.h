...
struct MenuContext_t {
	Vector2 pos = { GetSystemMetrics( SM_CXSCREEN ) / 2 - 648 / 2, GetSystemMetrics ( SM_CYSCREEN ) / 2 - 548 / 2 };

	// 660, 550
	Vector2 size = { 648, 548 };

	Vector2 NextGroupPos;
	std::stack< Vector2 > CursorPosStack;
...
namespace GUI {
	...
	Vector2 PopCursorPos( );
	Vector2 GetLastCursorPos( );
	void PushCursorPos( const Vector2& cursor_pos );
