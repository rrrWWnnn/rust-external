...
Vector2 last_cursor_pos;

Vector2 GUI::PopCursorPos( ) {
	if( ctx->CursorPosStack.empty( ) )
		return Vector2( );

	Vector2 ret = ctx->CursorPosStack.top( );
	ctx->CursorPosStack.pop( );
	last_cursor_pos = ret;
	return ret;
}

Vector2 GUI::GetLastCursorPos( ) {
	return last_cursor_pos;
}

void GUI::PushCursorPos( const Vector2& cursor_pos ) {
