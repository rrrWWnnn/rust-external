...
bool GUI::Controls::Listbox( const std::string& id, std::vector<std::string> elements, int* option, bool bSearchBar, int iSizeInElements ) {
	Vector2 CursorPos = GUI::PopCursorPos( );
	...
	bool bHoveredSearch = InputHelper::IsHoveringOverPoint( DrawPos - Vector2( 0, 18 ), Vector2( iWidth, 19 ) ) && InputHelper::IsHoveringOverPoint( ctx->ParentPos, ctx->ParentSize ) && bSearchBar;
	bool bHovered = InputHelper::IsHoveringOverPoint( DrawPos, Vector2( iWidth, kurwa * iSizeInElements ) ) && InputHelper::IsHoveringOverPoint( ctx->ParentPos, ctx->ParentSize );

	Render::DirectX::rect_fill( DrawPos, Vector2( iWidth, kurwa * iSizeInElements ), Color( 35, 35, 35 ) );
	...
	for( int i = 0; i < elements.size( ); i++ ) {
		...
		Vector2 OptionPos = DrawPos + Vector2( 0, iPos[ GUI::Hash( id ) ] + iSlider[ GUI::Hash( id ) ] );
		Vector2 OptionSize = Vector2( iWidth - ( scrollbar ? 6 : 0 ), kurwa );
		...
		if( bHovered && bIsInBounds ) {
			if( InputHelper::IsHoveringOverPoint( OptionPos, OptionSize - Vector2( 6, 0 ) ) && !bHoveredSearch && InputHelper::IsHoveringOverPoint( ctx->ParentPos, ctx->ParentSize ) && ctx->FocusedID == 0 ) {
		...
		if( bIsInBounds ) {
			if( *option != i ) {
				Render::DirectX::Fonts::menu.draw_text( elements[ i ].c_str( ), OptionPos + Vector2( 10, 4 ), Color( 200, 200, 200 ), 0 );
			}
			else {
				Render::DirectX::rect_fill( OptionPos, OptionSize, Color( 26, 26, 26 ) );
				Render::DirectX::Fonts::menu.draw_text( elements[ i ].c_str( ), OptionPos + Vector2( 10, 4 ), g_Vars.menu.menu_color.ToRegularColor ( ).OverrideAlpha( 255 * ctx->animation ), 0 );
			}
		}
	}
	...
	if( bDrawBottomArrow && ( vecFoundElements[ GUI::Hash( id ) ].size( ) > iSizeInElements ) ) {
		Render::DirectX::rect_fill( DrawPos + Vector2( iWidth, ( kurwa * iSizeInElements ) - 2 ) - Vector2( 17, 10 ), Vector2( 5, 1 ), Color( 0, 0, 0 ) );
		Render::DirectX::rect_fill( DrawPos + Vector2( iWidth, ( kurwa * iSizeInElements ) - 2 ) - Vector2( 17, 9 ), Vector2( 5, 1 ), Color( 204, 204, 204 ) );
		Render::DirectX::rect_fill( DrawPos + Vector2( iWidth, ( kurwa * iSizeInElements ) - 2 ) - Vector2( 16, 8 ), Vector2( 3, 1 ), Color( 204, 204, 204 ) );
		Render::DirectX::rect_fill( DrawPos + Vector2( iWidth, ( kurwa * iSizeInElements ) - 2 ) - Vector2( 15, 7 ), Vector2( 1, 1 ), Color( 204, 204, 204 ) );
	}

	// top arrow facing up
	if( bDrawTopArrow && iSlider[ GUI::Hash( id ) ] <= -20 ) {
		Render::DirectX::rect_fill( DrawPos + Vector2( iWidth - 20, 7 ) + Vector2( 5, 0 ), Vector2( 1, 1 ), Color( 204, 204, 204 ) );
		Render::DirectX::rect_fill( DrawPos + Vector2( iWidth - 20, 7 ) + Vector2( 4, 1 ), Vector2( 3, 1 ), Color( 204, 204, 204 ) );
		Render::DirectX::rect_fill( DrawPos + Vector2( iWidth - 20, 7 ) + Vector2( 3, 2 ), Vector2( 5, 1 ), Color( 204, 204, 204 ) );
		Render::DirectX::rect_fill( DrawPos + Vector2( iWidth - 20, 7 ) + Vector2( 3, 3 ), Vector2( 5, 1 ), Color( 0, 0, 0 ) );
	}
	...
	if( ( bHovered || ctx->FocusedID == GUI::Hash( id + "s" ) ) && vecFoundElements[ GUI::Hash( id ) ].size( ) > iSizeInElements && ( scrollbar_height != -1 && scrollbar_pos != -1 ) ) {
		...
		bool bHoveredScroll = InputHelper::IsHoveringOverPoint( DrawPos + Vector2( iWidth - 5, scrollbar_pos + 1 ), Vector2( 4, scrollbar_height ) );

		static float prev_mouse = 0.f;

		const auto diff = prev_mouse - InputHelper::GetMousePos( ).y;
		...
		prev_mouse = InputHelper::GetMousePos( ).y;
	...
	if( vecFoundElements[ GUI::Hash( id ) ].size( ) > iSizeInElements && ( scrollbar_height != -1 && scrollbar_pos != -1 ) ) {
		Render::DirectX::rect_fill( DrawPos + Vector2( iWidth - 6, 1 ), Vector2( 6, ( kurwa * iSizeInElements ) - 1 ), Color( 45, 45, 45 ) );
		Render::DirectX::rect_fill( DrawPos + Vector2( iWidth - 5, scrollbar_pos + 1 ), Vector2( 4, scrollbar_height ), ( InputHelper::IsHoveringOverPoint( DrawPos + Vector2( iWidth - 5, scrollbar_pos + 1 ), Vector2( 4, scrollbar_height ) ) || ctx->FocusedID == GUI::Hash( id + "s" ) ) ? Color( 80, 80, 80 ) : Color( 65, 65, 65 ) );
	}

	// search
	if( bSearchBar ) {
		Render::DirectX::rect_fill( DrawPos - Vector2( 0, 18 ), Vector2( iWidth, 19 ), Color( 25, 25, 25, 255 * ctx->animation ) );

		if( ctx->FocusedID == 0 ) {
			if( ctx->typing )
				ctx->typing = false;

			if( bHoveredSearch && InputHelper::Down( VK_LBUTTON ) )
				ctx->FocusedID = GUI::Hash( id + _ ( "search" ) );
		}
		else if( ctx->FocusedID == GUI::Hash( id + _ ( "search" ) ) ) {
			if( !ctx->typing )
				ctx->typing = true;
		...
		D3DVIEWPORT9 clip = { DrawPos.x, DrawPos.y - kurwa, Vector2( iWidth, kurwa ).x, Vector2( iWidth, kurwa ).y - 2, 0.f, 1.0f };
		Render::DirectX::set_scissor_rect( clip ); {
			Render::DirectX::Fonts::menu.draw_text( ( szText[ GUI::Hash( id ) ] + ( ctx->FocusedID == GUI::Hash( id + _ ( "search" ) ) ? '_' : ' ' ) ).c_str( ), DrawPos - Vector2( -4, 15 ), Color( 200, 200, 200 ), 0 );
		}
		Render::DirectX::reset_scissor_rect( );
	}

	if( bSearchBar ) {
		Render::DirectX::rect( DrawPos - Vector2( 0, 18 ), Vector2( iWidth, kurwa * iSizeInElements ) + Vector2( 0, 18 ), Color( 10, 10, 10 ) );
		Render::DirectX::rect( DrawPos - Vector2( 0, 18 ), Vector2( iWidth, 19 ), Color( 10, 10, 10 ) );
		Render::DirectX::rect( DrawPos - Vector2( 0, 18 ) + 1, Vector2( iWidth, 19 ) - 2, Color( 50, 50, 50 ) );
		Render::DirectX::rect( DrawPos - Vector2( 0, 18 ) + 2, Vector2( iWidth, 19 ) - 4, Color( 16, 16, 16 ) );
	}
	else {
		Render::DirectX::rect( DrawPos, Vector2( iWidth, kurwa * iSizeInElements ), Color( 10, 10, 10 ) );
	}

	GUI::PushCursorPos( CursorPos + Vector2( 0, ( kurwa * iSizeInElements ) + ( bSearchBar ? ( GUI::ObjectPadding( ) + kurwa ) : GUI::ObjectPadding( ) ) ) );
...
bool GUI::Controls::Luabox( const std::string& id, std::vector<std::string> elements, int* option, bool bSearchBar, int iSizeInElements ) {
	...
	GUI::PushCursorPos( CursorPos + Vector2( 0, ( 20 * iSizeInElements ) + ( bSearchBar ? ( GUI::ObjectPadding( ) + 20 ) : GUI::ObjectPadding( ) ) ) );
	return ctx->FocusedID == GUI::Hash( id );
}
#endif