...
bool GUI::Controls::Textbox( const std::string& name, std::string* input, int max_text ) {
	...
	Vector2 DrawSize( std::min( ( int )ctx->ParentSize.x - 90, 270 ), 18 );
	...
	Render::DirectX::Fonts::menu.draw_text( name, DrawPos + Vector2( 1, -3 ), Color( 203, 203, 203 ), 4 );

	DrawPos.y += 11;

	int iWidth = DrawSize.x;
	bool bHoveredSearch = InputHelper::IsHoveringOverPoint( DrawPos, Vector2( iWidth, DrawSize.y ) ) && InputHelper::IsHoveringOverPoint( ctx->ParentPos, ctx->ParentSize );

	// search
	Render::DirectX::rect_fill( DrawPos, Vector2( iWidth + 1, DrawSize.y ), Color( 25, 25, 25, 255 * ctx->animation ) );
			...
			if( InputHelper::Pressed( i ) ) {
				if( i != VK_BACK ) {
					if( i == VK_ESCAPE || i == VK_RETURN ) {
						...
						_szText[ GUI::Hash( name ) ] += ( ( InputHelper::Down( VK_SHIFT ) ) || GetKeyState( VK_CAPITAL ) ) ? i : i + 32;
					}
					else if( i >= '0' && i <= '9' || i == ' ' ) {
	...
	D3DVIEWPORT9 clip = { DrawPos.x, DrawPos.y + 1, Vector2( iWidth, DrawSize.y ).x, Vector2( iWidth, DrawSize.y ).y - 2, 0.f, 1.0f };

	Render::DirectX::rect( DrawPos, Vector2( iWidth, DrawSize.y ), Color( 10, 10, 10 ) );
	Render::DirectX::rect( DrawPos + 1, Vector2( iWidth, DrawSize.y ) - 2, Color( 60, 60, 60 ) );
	Render::DirectX::rect( DrawPos + 2, Vector2( iWidth, DrawSize.y ) - 4, Color( 16, 16, 16 ) );
	...
	*input = _szText[ GUI::Hash( name ) ];
	...
	Render::DirectX::set_scissor_rect( clip ); {
		Render::DirectX::Fonts::menu.draw_text( ( _szText[ GUI::Hash( name ) ] + (ctx->FocusedID == GUI::Hash(name) ? '_' : ' ') ).c_str( ), DrawPos + Vector2( 5, 3 ), Color( 200, 200, 200 ), 0 );
	}
	Render::DirectX::reset_scissor_rect( );

	PushCursorPos( CursorPos + Vector2( 0, DrawSize.y + Render::DirectX::Fonts::menu.measure_size( name ).y + GUI::ObjectPadding( ) ) );
