...
public:
	...
	void draw_text ( std::string text, Vector2 pos, Color c, int flags )
	{
		...
		{
			Draw ( text, pos + 1, Color ( 0, 0, 0, 255 /*c.a ( )*/ ), flags );
		...
		{
			// bleh
			for ( int x = -1; x <= 1; x++ ) for ( int y = -1; y <= 1; y++ ) Draw ( text, Vector2 ( pos.x + x, pos.y + y ), Color ( 0, 0, 0, 255 /*c.a ( )*/ ), flags );
		}

		Draw ( text, pos, c, flags );
	}

	Vector2 measure_size ( const std::string& text )
	{
		RECT rect;
